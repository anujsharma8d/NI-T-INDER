...
**Note:**
- When a mutual swipe occurs (both users swipe right on each other), a match is automatically created via a database trigger.
- The server also ensures a conversation record exists whenever a mutual right swipe is detected.
- The response will include additional flags when the swipe resulted in a match or opened a chat (this may happen even if the match already existed but a conversation was newly created):
  ```json
  {
    "swipe": { /* ... */ },
    "match_created": true,
    "match_id": "<match-id>",        // ID may be existing or newly created
    "conversation_id": "<conv-id>"  // always present when match_created=true
  }
  ```
  Clients can use `conversation_id` to open the chat immediately.
