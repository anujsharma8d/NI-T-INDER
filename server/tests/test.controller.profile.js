...
describe("Profile Controller", () => {
  ...
  describe("GET /profiles/:id/image", () => {
    it("should return image when lookup by profile id", async () => {
      const fakeImage = Buffer.from([0x01, 0x02]);
      mockDbPrepare.mockReturnValue({
        get: jest.fn().mockReturnValue({ profile_image: fakeImage }),
      });

      const response = await request(app).get("/profiles/profile-1/image");
      expect(response.status).toBe(200);
      expect(response.headers["content-type"]).toBe("image/png");
    });

    it("should return image when lookup by user id", async () => {
      const fakeImage = Buffer.from([0x01, 0x02]);
      mockDbPrepare.mockReturnValue({
        get: jest.fn().mockReturnValue({ profile_image: fakeImage }),
      });

      const response = await request(app).get("/profiles/user-123/image");
      expect(response.status).toBe(200);
    });
    ...
    it("should return 404 when image not found", async () => {
      ...
      const response = await request(app).get("/profiles/unknown/image");
      expect(response.status).toBe(404);
      expect(response.body.detail).toBe("image not found");
    });
  });
