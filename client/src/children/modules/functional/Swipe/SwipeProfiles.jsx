...
export default function SwipeProfiles({ActionButtons}) {
  ...
  const fetchProfiles = async (userId = currentUserId) => {
    try {
      setLoading(true)
      const response = await authFetch('/profiles/feed');
    ...
    const fetchImages = async () => {
      ...
      for (const p of profiles.slice(0, 3)) { // Only fetch for first 3 for perf
        if (p.id && !imageUrls[p.id]) {
          try {
            const res = await authFetch(`/profiles/${p.id}/image`);
            if (res.ok) {
  ...
  const sendSwipe = async (swipeeId, direction) => {
    try {
      const response = await authFetch('/swipes', {
        method: 'POST',
  ...
  const handleSwipe = async (direction) => {
    ...
    const result = await sendSwipe(currentProfile.id, direction)

    // If a match was created we inform the user and navigate to chat
    if (result?.match_created) {
      alert("It's a match! You can now message each other.");
      // navigate to chat and pass conversation id so chat page can open it
      if (result.conversation_id) {
        navigate('/chat', { state: { openConversationId: result.conversation_id } });
      } else {
        navigate('/chat');
      }
    }
    
    // Remove current profile and show next regardless
